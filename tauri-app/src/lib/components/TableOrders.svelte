<script lang="ts">
  ...
  import HashShortened from './HashShortened.svelte';
    ...
    <TableBody>
      {#each $ordersList.currentPage as order}
        <TableBodyRow on:click={() => goto(`/orders/${order.id}`)}>
          ...
          <TableBodyCell tdClass="break-all px-4 py-2"><HashShortened type={HashType.Identifier} value={order.id} /></TableBodyCell>
          <TableBodyCell tdClass="break-all px-4 py-2"><HashShortened type={HashType.Wallet} value={order.owner.id} /></TableBodyCell>
