...
export const useVaultBalanceChangesList = (id: string) =>  listStore<VaultBalanceChange>(
  ...
  (page) => invoke("vault_balance_changes_list", {subgraphArgs: { url: get(subgraphUrl)}, id, paginationArgs: { page: page+1, page_size: 10 } }),
  (path) => invoke("vault_balance_changes_list_write_csv", {path, subgraphArgs: { url: get(subgraphUrl)}, id})
);
