...
export const useVaultListBalanceChanges = (id: string) =>  listStore<VaultBalanceChange>(
  ...
  (page) => invoke("vault_list_balance_changes", {subgraphArgs: { url: get(subgraphUrl)}, id, paginationArgs: { page: page+1, page_size: 10 } }),
  (path) => invoke("vault_list_balance_changes_write_csv", {path, subgraphArgs: { url: get(subgraphUrl)}, id})
);
