...
export function validatedStringStore(key: string, defaultValue: string, handleIsValid: (value: string) => boolean) {
  const value = cachedWritableString(key, defaultValue);
  const isValid = derived(value, handleIsValid);
  const { subscribe } = derived([value, isValid], ([$value, $isValid]) => ({ value: $value, isValid: $isValid }));
