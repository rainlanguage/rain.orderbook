...
impl OrderCfg {
    pub fn validate_vault_id(value: &str) -> Result<U256, ParseOrderCfgError> {
        U256::from_str(value).map_err(ParseOrderCfgError::VaultParseError)
    }
    ...
    pub fn parse_network_key(
                    ...
                    orders_hash.get(&StrictYaml::String(order_key.to_string()))
                {
                    ...
                    if let Some(deployer_key) = optional_string(order_yaml, "deployer") {
                        ...
                        if let Some(ref existing_key) = network_key {
                            if *existing_key != key {
                                return Err(YamlError::ParseOrderCfgError(
                                    ParseOrderCfgError::DeployerNetworkDoesNotMatch {
                    ...
                    if let Some(orderbook_key) = optional_string(order_yaml, "orderbook") {
                        ...
                        if let Some(ref existing_key) = network_key {
                            if *existing_key != key {
                                return Err(YamlError::ParseOrderCfgError(
                                    ParseOrderCfgError::OrderbookNetworkDoesNotMatch {
                                        expected: existing_key.clone(),
                                        found: key.clone(),
                                    },
                                ));
                        ...
                        .enumerate()
                    {
                        ...
                        if let Ok(key) = res {
                            if let Some(ref existing_key) = network_key {
                                if *existing_key != key {
                                    return Err(YamlError::ParseOrderCfgError(
                                        ParseOrderCfgError::InputTokenNetworkDoesNotMatch {
                                            key: token_key,
                                            expected: existing_key.clone(),
                                            found: key.clone(),
                                        },
                            ...
                            .enumerate()
                    {
                        ...
                        if let Ok(key) = res {
                            if let Some(ref existing_key) = network_key {
                                if *existing_key != key {
                                    return Err(YamlError::ParseOrderCfgError(
                                        ParseOrderCfgError::OutputTokenNetworkDoesNotMatch {
                                            key: token_key,
                                            expected: existing_key.clone(),
                                            found: key.clone(),
                                        },
        ...
        Ok(network_key.ok_or(ParseOrderCfgError::NetworkNotFoundError(String::new()))?)
    }
            ...
            if let Ok(orders_hash) = require_hash(&document_read, Some("orders"), None) {
                for (key_yaml, order_yaml) in orders_hash {
                    ...
                    let order = OrderCfg {
                        ...
                        outputs,
                        network: network
                            .ok_or(ParseOrderCfgError::NetworkNotFoundError(String::new()))?,
                        deployer,
                        orderbook,
                    };
...
pub enum ParseOrderCfgError {
    #[error("Failed to parse deployer")]
    DeployerParseError(ParseDeployerCfgError),
    #[error("Failed to parse orderbook")]
    OrderbookParseError(ParseOrderbookCfgError),
    #[error("Failed to parse token")]
    TokenParseError(ParseTokenCfgError),
...
impl ParseOrderCfgError {
    pub fn to_readable_msg(&self) -> String {
        match self {
            ParseOrderCfgError::DeployerParseError(err) =>
                err.to_readable_msg(),
            ParseOrderCfgError::OrderbookParseError(err) =>
                err.to_readable_msg(),
            ParseOrderCfgError::TokenParseError(err) =>
                err.to_readable_msg(),
            ParseOrderCfgError::NetworkNotFoundError(_) =>
            ...
            ParseOrderCfgError::NetworkNotMatch =>
            ...
            ParseOrderCfgError::DeployerNetworkDoesNotMatch { expected, found } =>
            ...
            ParseOrderCfgError::OrderbookNetworkDoesNotMatch { expected, found } =>
            ...
            ParseOrderCfgError::InputTokenNetworkDoesNotMatch { key, expected, found } =>
            ...
            ParseOrderCfgError::OutputTokenNetworkDoesNotMatch { key, expected, found } =>
            ...
            ParseOrderCfgError::VaultParseError(err) =>
