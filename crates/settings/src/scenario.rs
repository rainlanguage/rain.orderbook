...
impl ScenarioCfg {
    pub fn validate_runs(value: &str) -> Result<u64, ParseScenarioCfgError> {
        value
            .parse::<u64>()
            .map_err(ParseScenarioCfgError::RunsParseError)
    }

    pub fn validate_blocks(value: &str) -> Result<BlocksCfg, ParseScenarioCfgError> {
        match serde_yaml::from_str::<BlocksCfg>(value) {
            Ok(blocks) => Ok(blocks),
            Err(_) => Err(ParseScenarioCfgError::BlocksParseError(value.to_string())),
        }
    }
    ...
    pub fn validate_scenario(
        ...
        for (k, v) in current_bindings {
            ...
            {
                if *parent_value != v {
                    return Err(YamlError::ParseScenarioCfgError(
                        ParseScenarioCfgError::ParentBindingShadowedError(k.to_string()),
                    ));
                }
            }
        ...
        if let Some(current_deployer) = current_deployer {
            if let Some(parent_deployer) = parent_scenario.deployer.as_ref() {
                if current_deployer.key != parent_deployer.key {
                    return Err(YamlError::ParseScenarioCfgError(
                        ParseScenarioCfgError::ParentDeployerShadowedError(
        ...
        scenarios.insert(
            ...
            ScenarioCfg {
                ...
                deployer: deployer
                    .clone()
                    .ok_or(ParseScenarioCfgError::DeployerNotFound(scenario_key))?,
            },
        );
        ...
        for document in &documents {
            ...
            if let Ok(scenarios_hash) = require_hash(&document_read, Some("scenarios"), None) {
                for (key_yaml, scenario_yaml) in scenarios_hash {
                    ...
                    if deployer.is_none() {
                        return Err(YamlError::ParseScenarioCfgError(
                            ParseScenarioCfgError::DeployerNotFound(scenario_key),
                        ));
                    }
                }
            }
...
pub enum ParseScenarioCfgError {
...
impl ParseScenarioCfgError {
    pub fn to_readable_msg(&self) -> String {
        match self {
            ParseScenarioCfgError::RunsParseError(err) =>
                format!("The 'runs' value in your scenario YAML configuration must be a valid number: {}", err),
            ParseScenarioCfgError::ParentBindingShadowedError(binding) =>
            ...
            ParseScenarioCfgError::ParentDeployerShadowedError(deployer) =>
            ...
            ParseScenarioCfgError::DeployerNotFound(scenario) =>
            ...
            ParseScenarioCfgError::ParentOrderbookShadowedError(orderbook) =>
...
"#;
        ...
        assert_eq!(
            error.to_string(),
            YamlError::ParseScenarioCfgError(ParseScenarioCfgError::ParentBindingShadowedError(
                "key1".to_string()
            ))
            .to_string()
        );
...
"#;
        ...
        assert_eq!(
            error.to_string(),
            YamlError::ParseScenarioCfgError(ParseScenarioCfgError::ParentDeployerShadowedError(
                "testnet".to_string()
            ))
            .to_string()
        );
