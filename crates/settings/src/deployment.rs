                ...
                for (key_yaml, deployment_yaml) in deployments_hash {
                    ...
                    if let Some(deployer) = &order.deployer {
                        if deployer != &scenario.deployer {
                            return Err(YamlError::ParseDeploymentCfgError(
                                ParseDeploymentCfgError::NoMatch,
                            ));
                        }
                    }
...
pub enum ParseDeploymentCfgError {
...
impl ParseDeploymentCfgError {
    pub fn to_readable_msg(&self) -> String {
        match self {
            ParseDeploymentCfgError::ScenarioNotFoundError(scenario) =>
            ...
            ParseDeploymentCfgError::OrderNotFoundError(order) =>
...
deployments:
    deployment1:
        ...
        assert_eq!(
            error.to_string(),
            YamlError::ParseDeploymentCfgError(ParseDeploymentCfgError::NoMatch).to_string()
        );
        assert_eq!(
            error.to_readable_msg(),
