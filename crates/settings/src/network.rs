...
impl NetworkCfg {
    ...
    pub fn validate_rpc(value: &str) -> Result<Url, ParseNetworkConfigError> {
        Url::parse(value).map_err(ParseNetworkConfigError::RpcParseError)
    }
    pub fn validate_chain_id(value: &str) -> Result<u64, ParseNetworkConfigError> {
        value
            .parse::<u64>()
            .map_err(ParseNetworkConfigError::ChainIdParseError)
    }
    pub fn validate_network_id(value: &str) -> Result<u64, ParseNetworkConfigError> {
        value
            .parse::<u64>()
            .map_err(ParseNetworkConfigError::NetworkIdParseError)
    }
...
pub enum ParseNetworkConfigError {
...
impl ParseNetworkConfigError {
    pub fn to_readable_msg(&self) -> String {
        match self {
            ParseNetworkConfigError::RpcParseError(err) => format!(
                "The RPC URL in your network configuration is invalid: {}",
                err
            ),
            ParseNetworkConfigError::ChainIdParseError(err) => format!(
                "The chain ID in your network configuration must be a valid number: {}",
                err
            ),
            ParseNetworkConfigError::NetworkIdParseError(err) => format!(
                "The network ID in your network configuration must be a valid number: {}",
                err
            ),
            ParseNetworkConfigError::RemoteNetworkKeyShadowing(key) => format!(
