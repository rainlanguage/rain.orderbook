...
impl TokenCfg {
    pub fn validate_address(value: &str) -> Result<Address, ParseTokenConfigError> {
        Address::from_str(value).map_err(ParseTokenConfigError::AddressParseError)
    }
    pub fn validate_decimals(value: &str) -> Result<u8, ParseTokenConfigError> {
        value
            .parse::<u8>()
            .map_err(ParseTokenConfigError::DecimalsParseError)
    }
            ...
            if let Some(yaml_cache) = &context.yaml_cache {
                for (key, token) in &yaml_cache.remote_tokens {
                    if tokens.contains_key(key) {
                        return Err(YamlError::ParseTokenConfigError(
                            ParseTokenConfigError::RemoteTokenKeyShadowing(key.clone()),
                        ));
                    }
...
pub enum ParseTokenConfigError {
...
impl ParseTokenConfigError {
    pub fn to_readable_msg(&self) -> String {
        match self {
            ...
            ParseTokenConfigError::RemoteTokenKeyShadowing(key) =>
