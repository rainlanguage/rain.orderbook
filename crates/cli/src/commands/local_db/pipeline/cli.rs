...
use crate::commands::local_db::pipeline::runner::executor::{
    ProducerJobFailure, ProducerRunReport, ProducerRunner,
};
...
mod tests {
    use super::*;
    use crate::commands::local_db::pipeline::runner::executor::{ExportMetadata, ProducerOutcome};
    ...
    fn default_out_root_is_local_db() {
        let command =
            SyncLocalDb::parse_from(["sync", "--settings-yaml", "test", "--api-token", "token"]);
    ...
    fn render_report_to_lists_failures() {
        let failure = ProducerJobFailure {
            chain_id: Some(1),
            orderbook_address: Some(address!("0000000000000000000000000000000000000fA1")),
        ...
        assert!(output.contains("chain 1 orderbook 0x0000000000000000000000000000000000000fA1"));
