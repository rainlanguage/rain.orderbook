...
use rain_orderbook_subgraph_client::{
  types::flattened::{TryIntoFlattenedError, OrderTakeFlattened},
...
impl Execute for CliOrderListTakes {
  async fn execute(&self) -> Result<()> {
      ...
      let order_takes = subgraph_args
          ...
          .await?;
      let order_takes_flattened: Vec<OrderTakeFlattened> =
          order_takes
              .into_iter()
              .map(|o| o.try_into())
...
fn build_table(order_take: Vec<OrderTakeFlattened>) -> Result<Table> {
  ...
  for order_take in order_take.into_iter() {
      table.add_row(vec![
          ...
          order_take.sender.0,
          format!("{} {}", order_take.input_display.0, order_take.input_token_symbol),
          format!("{} {}", order_take.output_display.0, order_take.output_token_symbol),
          format!("{} {}/{}", order_take.ioratio.0, order_take.input_token_symbol, order_take.output_token_symbol),
      ]);
  }
