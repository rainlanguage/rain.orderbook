...
mod tests {
    ...
    use crate::local_db::pipeline::runner::environment::{DumpFuture, ManifestFuture};
    use crate::local_db::pipeline::runner::{EnginePipelines, RunnerEnvironment, RunnerTarget};
    use crate::local_db::pipeline::{
        BootstrapConfig, BootstrapPipeline, BootstrapState, EventsPipeline, StatusBus, SyncConfig,
        TargetKey, TokensPipeline, WindowPipeline,
    };
    ...
    impl RecordingDb {
        ...
        fn set_text_error(&self, sql: &str, err: LocalDbQueryError) {
            self.inner
                .text_map
                .lock()
                .unwrap()
        ...
        fn text_calls(&self) -> Vec<String> {
            self.inner.text_calls.lock().unwrap().clone()
        }
