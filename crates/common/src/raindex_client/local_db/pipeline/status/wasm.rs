    ...
    async fn send(&self, phase: SyncPhase) -> Result<(), LocalDbError> {
        ...
        let status = match (self.latest_block.get(), self.synced_block.get()) {
            (Some(latest), Some(synced)) => {
                OrderbookSyncStatus::syncing_with_progress(ob_id.clone(), phase, latest, synced)
            }
...
mod tests {
    ...
    use crate::raindex_client::local_db::{LocalDbStatus, SchedulerState};
    ...
    fn orderbook_sync_status_active_with_blocks_sets_correct_fields() {
        let ob_id = test_ob_id();
        let status = OrderbookSyncStatus::active_with_blocks(
            ob_id.clone(),
