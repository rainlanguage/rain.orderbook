...
impl LocalDbQuery {
    ...
    pub async fn fetch_vaults_for_io_string(
        ...
        for (_, vault_id, token) in ios.iter() {
            if let Some(v) =
                Self::fetch_vault(db_callback, chain_id, orderbook_address, vault_id, token).await?
            {
                vaults.push(v);
            }
        }
...
mod tests {
    ...
    mod wasm_tests {
        ...
        async fn test_fetch_vault_parses_data() {
            ...
            let result = LocalDbQuery::fetch_vault(&callback, 1, "0xorder", "0x01", "0xaaa").await;
            assert!(result.is_ok());
        ...
        async fn test_fetch_vault_replaces_placeholders() {
            ...
            let _ = LocalDbQuery::fetch_vault(&callback, 137, "0xorder", "0xdead", "0xbeef").await;

            let sql = captured.borrow();
            assert!(sql.contains("'0xdead'"));
            assert!(sql.contains("'0xbeef'"));
            assert!(sql.contains("137"));
            assert!(sql.contains("'0xorder'"));
        ...
        async fn test_fetch_vaults_for_io_string_collects_multiple() {
            ...
            let result =
                LocalDbQuery::fetch_vaults_for_io_string(&callback, 1, "0xorder", &io).await;
            assert!(result.is_ok());
            let list = result.unwrap();
            assert_eq!(list.len(), 2);
            // both entries equal the sample because the callback returns the same row
            assert_eq!(list[0].chain_id, sample.chain_id);
