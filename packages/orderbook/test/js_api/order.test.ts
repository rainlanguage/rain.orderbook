...
describe("Rain Orderbook JS API Package Bindgen Tests - Order", async function () {
  ...
  it("should extend order with Rainlang string using extendOrder", async () => {
    ...
    try {
      const result = await extendOrder(order3);
      assert.ok(result, "Result should exist");
      assert.equal(result.order.id, order3.id);
      assert.equal(result.order.orderHash, order3.orderHash);
      assert.equal(result.order.owner, order3.owner);
