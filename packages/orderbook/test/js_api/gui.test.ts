...
`;
  ...
  describe("order operations tests", async () => {
    ...
    it("should populate vault ids if not set", async () => {
      mockServer
        .forPost("/rpc-url")
        .withBodyIncluding("0x82ad56cb")
        .thenSendJsonRpcResult(
      ...
      currentDeployment = gui.getCurrentDeployment();
      assert.equal(
        currentDeployment.deployment.order.inputs[0].vaultId,
        undefined
      );
      ...
      gui.populateVaultIds("0x123123123456456456");

      newCurrentDeployment = gui.getCurrentDeployment();
      assert.notEqual(
        newCurrentDeployment.deployment.order.inputs[0].vaultId,
        undefined
      );
    ...
    it("should throw error on deposit calldatas if vault ids are not set", async () => {
      // token1 info
      mockServer
        .forPost("/rpc-url")
        .once()
