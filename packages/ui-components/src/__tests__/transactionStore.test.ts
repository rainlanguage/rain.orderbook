...
describe('handleRemoveOrderTransaction', () => {
	...
	it('should successfully handle a remove order transaction', async () => {
		...
		// Mock explorer link
		const mockExplorerLink = 'https://etherscan.io/tx/0xremoveordertxhash';
		(getExplorerLink as Mock).mockResolvedValue(mockExplorerLink);
	...
	it('should handle user rejection of transaction', async () => {
		...
		// Mock user rejection of transaction
		...
		// Verify the chain switch was successful but the transaction was rejected
		expect(switchChain).toHaveBeenCalledWith(mockConfig, { chainId: mockChainId });
		expect(sendTransaction).toHaveBeenCalledWith(mockConfig, {
			to: mockOrderbookAddress,
			data: mockRemoveOrderCalldata
	...
	it('should handle transaction receipt failure', async () => {
		...
		// Verify the proper steps were executed
		expect(switchChain).toHaveBeenCalledWith(mockConfig, { chainId: mockChainId });
		expect(sendTransaction).toHaveBeenCalledWith(mockConfig, {
			to: mockOrderbookAddress,
			data: mockRemoveOrderCalldata
	...
	it('should handle explorer link error gracefully', async () => {
		// Mock successful chain switch
		(switchChain as Mock).mockResolvedValue({});
		...
		// Mock explorer link failure
		(getExplorerLink as Mock).mockResolvedValue('');

		// Mock successful transaction receipt
		(waitForTransactionReceipt as Mock).mockResolvedValue({});

		// Execute the function - should continue despite explorer link error
		await handleRemoveOrderTransaction({
			config: mockConfig,
			orderbookAddress: mockOrderbookAddress,
			removeOrderCalldata: mockRemoveOrderCalldata,
		...
		// Verify the transaction is still progressing - should be in PENDING_SUBGRAPH state
		const state = get(transactionStore);
		expect(state.status).toBe(TransactionStatus.PENDING_SUBGRAPH);
		expect(state.hash).toBe(mockTxHash);
		expect(state.explorerLink).toBe(''); // Empty explorer link because the API call failed
	});
});
...
describe('handleDepositOrWithdrawTransaction', () => {
	...
	const mockTransactionCalldata = '0xtransactioncalldata' as unknown as DepositCalldataResult | WithdrawCalldataResult;
	...
	it('should successfully handle a deposit transaction', async () => {
		...
		// Mock explorer link
		const mockExplorerLink = 'https://etherscan.io/tx/0xdeposittxhash';
		(getExplorerLink as Mock).mockResolvedValue(mockExplorerLink);
	...
	it('should successfully handle a withdrawal transaction', async () => {
		...
		// Mock explorer link
		const mockExplorerLink = 'https://etherscan.io/tx/0xwithdrawtxhash';
		(getExplorerLink as Mock).mockResolvedValue(mockExplorerLink);
	...
	it('should handle main transaction receipt failure for deposit', async () => {
		...
		// Mock explorer link
		(getExplorerLink as Mock).mockResolvedValue('https://etherscan.io/tx/0xdeposittxhash');
	...
	it('should handle main transaction receipt failure for withdrawal', async () => {
		...
		// Mock explorer link
		(getExplorerLink as Mock).mockResolvedValue('https://etherscan.io/tx/0xwithdrawtxhash');
