<script lang="ts">
	...
	async function executeQuery() {
		...
		const result = await localDbClient.query(sqlQuery);
		if (result.error) {
			error = result.error.readableMsg;
			isLoading = false;
			return;
		}
	...
	>
		...
		<div class="mb-4">
			...
			>
				...
				<Button on:click={executeQuery} color="blue" size="lg">
