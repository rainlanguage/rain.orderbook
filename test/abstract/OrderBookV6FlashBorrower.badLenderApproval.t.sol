...
import {MaliciousOrderBook} from "test/util/concrete/MaliciousOrderBook.sol";

/// A malicious orderBook passed to arb4 gets forceApprove before the
/// BadLender check in onFlashLoan fires. The entire tx reverts atomically,
/// so the approvals never persist.
contract OrderBookV6FlashBorrowerBadLenderApprovalTest is Test {
    function testBadLenderRevertsWithApproval() external {
        ...
        MaliciousOrderBook malicious = new MaliciousOrderBook();
        ...
        // arb4 reverts with BadLender because the malicious orderbook is not
        // the deterministic orderbook address.
