...
contract GenericPoolOrderBookV6FlashBorrowerExchangeRevertTest is Test {
    function testExchangeRevertPropagates() external {
        ...
        bytes memory exchangeData = abi.encode(
            address(exchange),
            address(exchange),
            abi.encodeCall(RevertingExchange.swap, (IERC20(address(outputToken)), IERC20(address(inputToken)), 100e18))
        );
