...
contract OrderBookDepositTest is OrderBookExternalMockTest {
    ...
    function testDepositSimple(address depositor, uint256 vaultId, uint256 amount) external {
        ...
        iOrderbook.deposit2(address(iToken0), vaultId, amount, new TaskV2[](0));
    ...
    function testDepositZero(address depositor, uint256 vaultId) external {
        ...
        iOrderbook.deposit2(address(iToken0), vaultId, 0, new TaskV2[](0));
    }
    ...
    function testDepositGas00() external {
        ...
        iOrderbook.deposit2(address(iToken0), 0, 1, new TaskV2[](0));
        vm.mockCall(
            address(iToken0),
            abi.encodeWithSelector(IERC20.transferFrom.selector, address(this), address(iOrderbook), 1),
            abi.encode(true)
    ...
    function testDepositMany(Action[] memory actions) external {
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            emit DepositV2(actions[i].depositor, actions[i].token, actions[i].vaultId, uint256(actions[i].amount));
            vm.record();
            vm.recordLogs();
            iOrderbook.deposit2(actions[i].token, actions[i].vaultId, actions[i].amount, new TaskV2[](0));
    ...
    function testDepositEvent(address depositor, uint256 vaultId, uint256 amount) external {
        ...
        vm.expectEmit(false, false, false, true);
        emit DepositV2(depositor, address(iToken0), vaultId, amount);
        iOrderbook.deposit2(address(iToken0), vaultId, amount, new TaskV2[](0));
    }
    ...
    function testDepositReentrancy(
        ...
        reenteroor.reenterWith(
            abi.encodeWithSelector(IOrderBookV4.deposit2.selector, reToken, reVaultId, reAmount, new TaskV2[](0))
        );
        vm.expectRevert(bytes("ReentrancyGuard: reentrant call"));
        iOrderbook.deposit2(address(reenteroor), vaultId, amount, new TaskV2[](0));
    }
    ...
    function testDepositOverflow(address depositor, uint256 vaultId, uint256 amountOne, uint256 amountTwo) external {
        ...
        iOrderbook.deposit2(address(iToken0), vaultId, amountOne, new TaskV2[](0));
        ...
        iOrderbook.deposit2(address(iToken0), vaultId, amountTwo, new TaskV2[](0));
    }
