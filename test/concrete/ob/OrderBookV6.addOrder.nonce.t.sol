...
contract OrderBookV6AddOrderNonceTest is OrderBookV6ExternalRealTest {
    ...
    function testAddOrderNonceSameNonceDifferentOrderStateChange(
        ...
        vm.prank(owner);
        bool stateChange = IRaindexV6(LibOrderBookDeploy.ORDERBOOK_DEPLOYED_ADDRESS).addOrder4(config0, new TaskV2[](0));
        assert(stateChange);
        assert(
            IRaindexV6(LibOrderBookDeploy.ORDERBOOK_DEPLOYED_ADDRESS)
                .orderExists(
        ...
        vm.prank(owner);
        stateChange = IRaindexV6(LibOrderBookDeploy.ORDERBOOK_DEPLOYED_ADDRESS).addOrder4(config1, new TaskV2[](0));
        assert(stateChange);
        assert(
            IRaindexV6(LibOrderBookDeploy.ORDERBOOK_DEPLOYED_ADDRESS)
                .orderExists(
