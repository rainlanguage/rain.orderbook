...
pub async fn get_remove_order(rpc_url: String, order_id: String) -> anyhow::Result<Order> {
    let variables = remove_order::Variables { id: Some(order_id) };
    ...
    let order_data: remove_order::RemoveOrderOrders =
        response_body.data.unwrap().orders.pop().unwrap();
